    ...
    then
        ...
        then
            ...
            do
                ...
                date --utc -d "+3 hours"
                curl -sL --proxy http://"${arr[$((0 + $RANDOM % 399))]}" https://discord.com/api/v10/channels/1075371070493831259/messages -X POST -H "Content-Type: application/json" -H "Authorization: $TOKEN" -d '{"content": "!faucet '"$wallet"'"}'
                sleep 4
            done < "$invento"
            end_time=$(date +%s)
