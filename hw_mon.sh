...
then
curl --header 'Content-Type: application/json' --request 'POST' --data '{"chat_id":"'"$CHAT_ID_ALARM"'","text":"<b>'🔴"$HOST_NAME":"$INFO_ALARM1"'</b>'" [$IP]"'<code>
CPU  >>> ['"$CPU"']
RAM  >>> ['"$USED_RAM"']
Disk >>> ['"$USED_DISK"']
...
then
curl --header 'Content-Type: application/json' --request 'POST' --data '{"chat_id":"'"$CHAT_ID_HARDINFO"'","text":"<b>'🟢"$HOST_NAME"'</b>'" [$IP]"'<code>
Used_CPU >> ['"$CPU"']
